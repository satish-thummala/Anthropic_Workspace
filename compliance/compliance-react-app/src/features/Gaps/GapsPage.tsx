...
import type { ToastFn, ApiGap, ApiGapStatus } from '../../types/compliance.types';
...
function GapCard({
  ...
  async function handleStatusChange() {
    ...
    try {
      ...
      toast(`Gap marked as ${(STATUS_MAP as any)[next]?.label ?? next}`, 'success');
    } catch {
  ...
  const canAdvance = gap.status !== 'resolved' && gap.status !== 'accepted_risk';

  return (
    <div className="gap-item" style={{ borderLeftColor: SEV_COLORS[gap.severity] }}>
      <div className="gap-header">
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 5, alignItems: 'center', flexWrap: 'wrap' }}>
            <span
              className="fw-badge"
              style={{ background: `${gap.frameworkColor}18`, color: gap.frameworkColor,
                       border: `1px solid ${gap.frameworkColor}40` }}
            >
              {gap.frameworkCode}
            </span>
            <span className="badge" style={{ background: SEV_BG[gap.severity], color: SEV_COLORS[gap.severity], fontSize: 11 }}>
              {gap.severity}
            </span>
            ...
            {gap.controlCategory && (
              <span style={{ fontSize: 11, color: 'var(--text3)' }}>{gap.controlCategory}</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          {gap.status === 'resolved' && (
            <Icons.Check style={{ width: 18, height: 18, color: '#16A34A', marginTop: 2 }} />
          )}
          {gap.status === 'accepted_risk' && (
            <span style={{ fontSize: 11, color: '#8B5CF6', fontWeight: 600 }}>Accepted</span>
          )}
        </div>
      </div>
      ...
      {gap.remediationNotes && (
        <div style={{ marginTop: 8, padding: '8px 12px', background: '#F0FDF4', borderRadius: 8,
                      border: '1px solid #BBF7D0', fontSize: 12, color: '#065F46' }}>
          <span style={{ fontWeight: 600 }}>Resolution note: </span>{gap.remediationNotes}
        </div>
      )}

      {gap.assignedToName && (
        <div style={{ marginTop: 6, fontSize: 12, color: 'var(--text3)' }}>
          Assigned to <span style={{ fontWeight: 600, color: 'var(--text2)' }}>{gap.assignedToName}</span>
...
export function GapsPage({ toast }: Props) {
  ...
  const filtered = useMemo(() => gaps.filter(g => {
    ...
    if (search.trim()) {
      ...
      if (!g.controlCode.toLowerCase().includes(q) &&
  ...
  async function handleRunAnalysis() {
    ...
    try {
      ...
      toast('Failed to refresh gaps', 'error');
    } finally {
  ...
  if (loading && gaps.length === 0) return (
  ...
  if (error) return (
    <div className="slide-in">
      <div className="card" style={{ textAlign: 'center', padding: 40, color: '#DC2626' }}>
        ...
        <button className="btn btn-secondary btn-sm" style={{ marginTop: 16 }} onClick={reload}>Retry</button>
      </div>
    </div>
  );
    ...
    <div className="slide-in">
        ...
        {(Object.entries(counts) as [string, number][]).map(([sev, count]) => (
          <div
            ...
            style={{
              ...
              outline: sevFilter === sev ? `2px solid ${SEV_COLORS[sev as keyof typeof SEV_COLORS]}` : 'none',
              outlineOffset: 2,
            }}
            onClick={() => setSevFilter(prev => prev === sev ? 'all' : sev)}
          >
            <div style={{ fontSize: 26, fontWeight: 800, color: SEV_COLORS[sev as keyof typeof SEV_COLORS] }}>
              {count}
            </div>
      ...
      {stats && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap' }}>
          {[
            { label: 'Open',          key: 'open',          val: stats.totalOpen,         color: '#EF4444', bg: '#FEF2F2' },
            { label: 'In Progress',   key: 'in_progress',   val: stats.totalInProgress,   color: '#3B82F6', bg: '#EFF6FF' },
            { label: 'Resolved',      key: 'resolved',      val: stats.totalResolved,     color: '#22C55E', bg: '#F0FDF4' },
            { label: 'Accepted Risk', key: 'accepted_risk', val: stats.totalAcceptedRisk, color: '#8B5CF6', bg: '#F5F3FF' },
          ].map(s => (
            <div
              key={s.key}
      ...
      <div style={{ fontSize: 13, color: 'var(--text3)', marginBottom: 12 }}>
        Showing <strong style={{ color: 'var(--text1)' }}>{filtered.length}</strong> gap{filtered.length !== 1 ? 's' : ''}
        {gaps.length !== filtered.length && ` (filtered from ${gaps.length})`}
      </div>

      {/* Gap cards */}
      {filtered.map(gap => (
        <GapCard key={gap.id} gap={gap} onStatusChange={handleGapUpdated} toast={toast} />
      ))}
      ...
      {filtered.length === 0 && !loading && (
        <div className="card" style={{ textAlign: 'center', padding: 48 }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 600 }}>
            {gaps.length === 0 ? 'All controls are covered — great work!' : 'No gaps match your filters'}
          </div>
          <div style={{ color: 'var(--text2)', marginTop: 4, fontSize: 13 }}>
            {gaps.length === 0 ? 'Run the Gap Analysis to check for new gaps' : 'Try adjusting your search criteria'}
          </div>
        </div>
      )}
    </div>
