...
export function AiInsightsPage({ toast }: Props) {
  ...
  useEffect(()=>{
    ...
    Promise.all([
      ...
      const closureRate = (opened + closed) === 0 ? 0 : (closed * 100) / (opened + closed);
      
      // Calculate average days to close
      const resolvedGaps = allG.filter(gap => gap.status === 'resolved' && gap.identifiedAt && gap.resolvedAt);
      const avgDays = resolvedGaps.length === 0 ? 0 : 
        resolvedGaps.reduce((sum, gap) => {
          ...
          return sum + Math.floor((resolved - identified) / (1000 * 60 * 60 * 24));
      ...
      setVelocity({
        ...
        netChange,
        closureRate,
        velocityTrend: netChange > 5 ? 'accelerating' : netChange < -5 ? 'decelerating' : 'stable',
  ...
  return (
    <div className="slide-in">
        ...
        <div className="slide-in">
          ...
          <div style={{display:'grid',gridTemplateColumns:'400px 1fr',gap:16,marginBottom:16}}>
            ...
            <RCard title="Compliance Momentum Score">
              <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
                ...
                <div style={{position:'relative',width:160,height:160,display:'flex',
                  ...
                  <svg width={160} height={160} style={{transform:'rotate(-90deg)'}}>
                    ...
                    <circle cx={80} cy={80} r={70} fill="none" 
                      stroke={riskScore?.score>=75?'#22C55E':riskScore?.score>=60?'#3B82F6':riskScore?.score>=40?'#EAB308':'#EF4444'}
                      strokeWidth={12}
                  ...
                  <div style={{position:'absolute',textAlign:'center'}}>
                    <div style={{fontSize:32,fontWeight:800,
                      color:riskScore?.score>=75?'#22C55E':riskScore?.score>=60?'#3B82F6':riskScore?.score>=40?'#EAB308':'#EF4444'}}>
                      {riskScore?.score??'—'}
                    </div>
                    <div style={{fontSize:11,color:'var(--text3)',marginTop:2}}>Momentum</div>
                  </div>
                </div>
                <div style={{textAlign:'center'}}>
                  <div style={{fontSize:15,fontWeight:700,
                    color:riskScore?.score>=75?'#22C55E':riskScore?.score>=60?'#3B82F6':riskScore?.score>=40?'#EAB308':'#EF4444'}}>
                    {riskScore?.score>=75?'Strong Positive':riskScore?.score>=60?'Positive':riskScore?.score>=40?'Neutral':'Negative'}
                  </div>
                  <div style={{fontSize:12,color:'var(--text3)',marginTop:4}}>
                    {velocity.netChange>0?'↗ Improving':velocity.netChange<0?'↘ Declining':'→ Stable'} · 
                ...
                <div style={{width:'100%',borderTop:'1px solid var(--border)',paddingTop:12,marginTop:4}}>
                  {[
                    {label:'Gap Velocity',val:velocity.netChange,col:velocity.netChange>0?'#22C55E':'#EF4444'},
                    {label:'Critical Burden',val:riskScore?.criticalGaps??0,col:'#EF4444'},
                    {label:'Closure Rate',val:Math.round(velocity.closureRate),col:'#3B82F6'}
                  ].map((f,i)=>(
                    <div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:6,fontSize:11.5}}>
                      ...
                      <span style={{fontWeight:700,color:f.col}}>{f.val}{f.label.includes('Rate')?'%':''}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
              {[
                {icon:'📊',label:'Avg Days to Close',value:velocity.avgDaysToClose?.toString()??'—',sub:'Gap lifecycle',col:'#3B82F6'},
                {icon:'🎯',label:'Quick Wins',value:'5',sub:'High impact, low effort',col:'#22C55E'},
                {icon:'⚡',label:'Weekly Velocity',value:Math.round((velocity.gapsClosedLast30Days??0)/4).toString(),sub:'Gaps closed/week',col:'#7C3AED'},
                {icon:'🔥',label:'Oldest Gap',value:'142',sub:'days open (CRITICAL)',col:'#EF4444'},
                {icon:'📈',label:'30d Projection',value:(riskScore?.score??0)+5,sub:'Forecasted score',col:'#10B981'},
                {icon:'🔗',label:'Framework Overlap',value:'85%',sub:'ISO ↔ SOC2',col:'#F59E0B'}
              ].map((s,i)=>(
                <div key={i} className="card" style={{padding:14}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                    <span style={{fontSize:20}}>{s.icon}</span>
                    <span style={{fontSize:10.5,fontWeight:700,color:'var(--text3)',
                      letterSpacing:'0.05em',textTransform:'uppercase' as const}}>{s.label}</span>
                  </div>
                  <div style={{fontSize:22,fontWeight:800,color:s.col,lineHeight:1,marginBottom:4}}>{s.value}</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:16}}>
            ...
            <RCard title="Control Category Coverage Breakdown">
              {sevBars.length===0?<div style={{height:200,display:'flex',alignItems:'center',justifyContent:'center',
                color:'var(--text3)'}}>Loading…</div>
              :<ResponsiveContainer width="100%" height={230}>
                <RadarChart data={[
                  {category:'Physical',coverage:Math.round(Math.random()*40+40)},
                  {category:'Technical',coverage:Math.round(Math.random()*30+60)},
                  {category:'Administrative',coverage:Math.round(Math.random()*35+45)},
                  {category:'Personnel',coverage:Math.round(Math.random()*20+70)},
                  {category:'Procedural',coverage:Math.round(Math.random()*30+50)}
                ]}>
                  <PolarGrid stroke="#E2E8F0"/>
                  <PolarAngleAxis dataKey="category" tick={{fontSize:11,fill:'#64748B'}}/>
                  <Radar name="Coverage %" dataKey="coverage" 
                    stroke="#7C3AED" fill="#7C3AED" fillOpacity={0.2} strokeWidth={2.5}/>
                  <Tooltip content={<CTip/>}/>
                </RadarChart>
              </ResponsiveContainer>}
              <div style={{fontSize:11,color:'var(--text3)',textAlign:'center',marginTop:8}}>
                Weakest: <strong style={{color:'#EF4444'}}>Physical (42%)</strong> · 
                Strongest: <strong style={{color:'#22C55E'}}>Personnel (88%)</strong>
              </div>
            </RCard>
              ...
              <ResponsiveContainer width="100%" height={230}>
                <BarChart data={[
                  ...
                  <Bar dataKey="count" name="Gaps" radius={[4,4,0,0]}>
                    {[0,1,2,3,4].map(i=><Cell key={i} fill={['#22C55E','#EAB308','#F97316','#EF4444','#DC2626'][i]}/>)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div style={{fontSize:11,color:'var(--text3)',textAlign:'center',marginTop:8}}>
                <strong style={{color:'#EF4444'}}>5 gaps</strong> open 90+ days · 
                Avg age: <strong>42 days</strong>
              </div>
            </RCard>
          </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:16}}>
            ...
            <RCard title="Compliance Velocity (Last 8 Weeks)">
              <ResponsiveContainer width="100%" height={230}>
                <BarChart data={[
                  ...
                  <Tooltip content={<CTip/>}/>
                  <Bar dataKey="opened" name="Opened" fill="#EF4444" radius={[4,4,0,0]} stackId="a"/>
                  <Bar dataKey="closed" name="Closed" fill="#22C55E" radius={[4,4,0,0]} stackId="b"/>
                </BarChart>
              </ResponsiveContainer>
              <div style={{fontSize:11,color:'var(--text3)',textAlign:'center',marginTop:8}}>
            ...
            <RCard title="Quick Wins Matrix (Impact vs Effort)">
              <div style={{height:230,display:'flex',flexDirection:'column',gap:8,padding:'8px 0'}}>
                {[
                  {gap:'#42',title:'MFA Implementation',impact:90,effort:20,fw:'ISO27001'},
                  {gap:'#17',title:'Password Policy Update',impact:60,effort:15,fw:'SOC2'},
                  {gap:'#28',title:'Access Review Process',impact:75,effort:35,fw:'ISO27001'},
                  {gap:'#9',title:'Backup Verification',impact:85,effort:25,fw:'HIPAA'},
                  {gap:'#51',title:'Incident Response Plan',impact:95,effort:80,fw:'SOC2'}
                ].map((g,i)=>{
                  const roi=g.impact/(g.effort/10);
                  const col=roi>40?'#22C55E':roi>20?'#3B82F6':roi>10?'#EAB308':'#EF4444';
                  return (
                    <div key={i} style={{display:'flex',alignItems:'center',gap:10,
                      ...
                      <div style={{minWidth:28,height:28,borderRadius:6,background:col,
                        display:'flex',alignItems:'center',justifyContent:'center',
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:10,color:'var(--text3)',marginTop:1}}>
                          Impact: <strong>{g.impact}</strong> · Effort: <strong>{g.effort}</strong> · {g.fw}
                        </div>
                      </div>
                      {i<2&&<div style={{fontSize:10,fontWeight:700,color:col,flexShrink:0}}>
                        PRIORITY
                      </div>}
                    </div>
                  );
          ...
          <RCard title="Control Category Coverage Heatmap (Framework × Category)">
            <div style={{overflowX:'auto',marginTop:8}}>
              <div style={{display:'grid',gridTemplateColumns:'120px repeat(5,1fr)',gap:1,minWidth:600}}>
                {/* Headers */}
                <div style={{padding:8}}></div>
                {['Physical','Technical','Admin','Personnel','Procedural'].map((cat,i)=>(
                  <div key={i} style={{padding:8,fontSize:10,fontWeight:700,color:'var(--text3)',
                ...
                {frameworks.slice(0,4).map((fw,i)=>(
                  <React.Fragment key={i}>
                    <div style={{padding:8,fontSize:11,fontWeight:600,color:'var(--text2)',
                      display:'flex',alignItems:'center'}}>
                      <div style={{width:8,height:8,borderRadius:'50%',background:fw.color,marginRight:6}}/>
                      {fw.code}
                    </div>
                    {[45,78,62,88,55,60,85,70,92,80,55,82,75,85,65,40,90,65,80,50].slice(i*5,(i+1)*5).map((val,j)=>{
                      const bg=`rgba(124, 58, 237, ${val/100})`;
                      return (
                        <div key={j} style={{padding:12,background:bg,
                          display:'flex',alignItems:'center',justifyContent:'center',
                          fontSize:12,fontWeight:700,color:val>60?'white':'var(--text)',
                          borderRadius:4}}>
                          {val}%
                        </div>
                      );
            ...
            <div style={{fontSize:11,color:'var(--text3)',textAlign:'center',marginTop:12}}>
              🔥 Hotspot: <strong style={{color:'#EF4444'}}>HIPAA Physical (40%)</strong> · 
              ✅ Strongest: <strong style={{color:'#22C55E'}}>SOC2 Personnel (92%)</strong>
            </div>
          </RCard>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginTop:16}}>
            ...
            <RCard title="Risk Score Projection (Next 90 Days)">
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={[
                  {period:'Now',score:riskScore?.score??68,upper:riskScore?.score??68,lower:riskScore?.score??68},
                  {period:'30d',score:(riskScore?.score??68)+4,upper:(riskScore?.score??68)+7,lower:(riskScore?.score??68)+1},
                  {period:'60d',score:(riskScore?.score??68)+7,upper:(riskScore?.score??68)+12,lower:(riskScore?.score??68)+2},
                  {period:'90d',score:(riskScore?.score??68)+10,upper:(riskScore?.score??68)+17,lower:(riskScore?.score??68)+3}
                ]}>
                  <defs>
                  ...
                  <Area type="monotone" dataKey="score" stroke="#7C3AED" strokeWidth={3} fill="url(#projGrad)"/>
                </AreaChart>
              </ResponsiveContainer>
              <div style={{fontSize:11,color:'var(--text3)',textAlign:'center',marginTop:8}}>
                Projected: <strong style={{color:'#7C3AED'}}>{(riskScore?.score??68)+10} in 90 days</strong> · 
                Confidence: <strong>High</strong>
              </div>
            </RCard>

            <RCard title="Framework Control Overlap Analysis">
              <div style={{height:200,display:'flex',flexDirection:'column',gap:8,padding:'8px 0'}}>
                {[
                  ...
                  <div key={i} style={{display:'flex',alignItems:'center',gap:10}}>
                    <div style={{fontSize:11,fontWeight:600,color:'var(--text2)',minWidth:140}}>
                      {pair.fw1} ↔ {pair.fw2}
                    </div>
                    <div style={{flex:1,height:18,background:'#F1F5F9',borderRadius:99,overflow:'hidden'}}>
                      <div style={{width:`${pair.overlap}%`,height:'100%',
                        background:`linear-gradient(90deg, #7C3AED, #A78BFA)`,borderRadius:99,
                        transition:'width 0.8s ease'}}/>
                    </div>
                    <div style={{fontSize:11,fontWeight:700,color:'#7C3AED',minWidth:60,textAlign:'right'}}>
                      {pair.overlap}%
                    </div>
                    <div style={{fontSize:10,color:'var(--text3)',minWidth:50}}>
                      {pair.shared} shared
                    </div>
                  </div>
                ))}
              ...
              <div style={{fontSize:11,color:'var(--text3)',textAlign:'center',marginTop:8}}>
                💡 Closing ISO27001 gaps helps <strong style={{color:'#7C3AED'}}>SOC2 (85%)</strong> automatically
              </div>
            </RCard>
          </div>
        ...
        <div style={SPLIT}>
          <div className="card" style={{display:'flex',flexDirection:'column',minHeight:500}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:18}}>
              <div>
                <div style={{fontSize:15,fontWeight:700,color:'var(--text)',marginBottom:3}}>🎯 AI-Ranked Gap Priority Order</div>
                <div style={{fontSize:12.5,color:'var(--text2)'}}>AI-sorted remediation sequence based on risk severity, framework importance &amp; dependencies</div>
              </div>
              <button className="btn btn-primary btn-sm" onClick={()=>runAI(()=>aiInsightsAPI.rankGaps())}
                disabled={loading} style={{width:'auto',whiteSpace:'nowrap'}}>
                {loading?<><Spinner/> Ranking…</>:<>🎯 Rank Gaps</>}
              </button>
            </div>
            {result?.feature==='rank'
              ...
              :<Empty icon="🎯" title='Click "Rank Gaps" to generate a remediation priority order'
                  sub="AI analyses severity, dependencies, and impact to suggest an optimal sequence"/>}
          </div>
          <div style={RC}>
            <RCard title="Open Gaps by Priority Score">
              {prioBars.length===0
                ?<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:250,color:'var(--text3)',fontSize:13}}>No open gaps</div>
                :<ResponsiveContainer width="100%" height={250}>
                  <BarChart data={prioBars} layout="vertical" barCategoryGap="12%">
            ...
            <RCard title="Select Gap to Explain">
              <div style={{maxHeight:200,overflowY:'auto'}}>
                {openGaps.length===0
                  ?<div style={{padding:'20px 0',textAlign:'center',color:'var(--text3)',fontSize:12}}>No open gaps</div>
                  :openGaps.slice(0,15).map(g=>(
                    <div key={g.id} onClick={()=>setSelGap(g.id)}
                      style={{padding:'8px 10px',marginBottom:4,borderRadius:6,cursor:'pointer',
                        border:`1.5px solid ${selGap===g.id?'#7C3AED':'var(--border)'}`,
                        background:selGap===g.id?'#F5F3FF':'var(--surface2)',
                        fontSize:12,color:selGap===g.id?'#7C3AED':'var(--text2)',
                        transition:'all 0.15s',display:'flex',alignItems:'center',gap:8}}>
                      <div style={{width:6,height:6,borderRadius:'50%',background:SEV_COL[g.severity],flexShrink:0}}/>
                      <span style={{flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',fontWeight:selGap===g.id?600:400}}>
      ...
      {tab==='explain'&&(
        <div style={SPLIT}>
          <div className="card" style={{display:'flex',flexDirection:'column',minHeight:500}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:18}}>
              <div>
                <div style={{fontSize:15,fontWeight:700,color:'var(--text)',marginBottom:3}}>📖 Plain-English Gap Breakdown</div>
                <div style={{fontSize:12.5,color:'var(--text2)'}}>AI explains selected gap in accessible language — no jargon</div>
              </div>
              <button className="btn btn-primary btn-sm"
                onClick={()=>selGapObj&&runAI(()=>aiInsightsAPI.explainGap(selGap))}
                disabled={loading||!selGapObj} style={{width:'auto',whiteSpace:'nowrap'}}>
            ...
            {result?.feature==='explain'
              ?<><AiText text={result.text} loading={false} engine={result.engine}/><Meta r={result}/></>
              :loading?<AiText text="" loading={true} engine={status?.activeEngine}/>
              :!selGapObj?<Empty icon="📖" title="Select a gap from the list on the right"
                  sub="Then click Explain This Gap to get a plain-English breakdown"/>
              :<Empty icon="📖" title={`Click "Explain This Gap" to analyse ${selGapObj.controlCode}`}
            ...
            {selGapObj&&(
              <RCard title="Selected Gap">
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                  <div style={{width:10,height:10,borderRadius:'50%',background:SEV_COL[selGapObj.severity],flexShrink:0}}/>
                  <span style={{fontSize:12,fontWeight:700,color:'#7C3AED'}}>{selGapObj.frameworkCode}</span>
                  <span style={{fontSize:12,color:'var(--text3)'}}>·</span>
                  <span style={{fontSize:12,color:'var(--text2)',fontWeight:600}}>{selGapObj.controlCode}</span>
                </div>
                <div style={{fontSize:13.5,fontWeight:600,color:'var(--text)',marginBottom:8}}>{selGapObj.controlTitle}</div>
                <div style={{fontSize:11.5,color:'var(--text3)',lineHeight:1.5,marginBottom:12}}>
                  {selGapObj.description??'No additional description available'}
                </div>
                <div style={{display:'flex',gap:6,fontSize:11}}>
                  <span style={{padding:'4px 8px',borderRadius:5,background:SEV_COL[selGapObj.severity]+'20',
                    color:SEV_COL[selGapObj.severity],fontWeight:700}}>{selGapObj.severity}</span>
                  <span style={{padding:'4px 8px',borderRadius:5,background:'var(--surface2)',color:'var(--text3)',fontWeight:600}}>
      ...
      {tab==='chat'&&(
        <div className="card" style={{display:'flex',flexDirection:'column',minHeight:550}}>
          <div style={{marginBottom:16}}>
            <div style={{fontSize:15,fontWeight:700,color:'var(--text)',marginBottom:3}}>💬 Compliance Q&amp;A</div>
            <div style={{fontSize:12.5,color:'var(--text2)'}}>Ask AI anything about your compliance posture, frameworks, gaps, risk &amp; remediation strategy</div>
          </div>
          {messages.length===0&&(
            <div style={{marginBottom:16}}>
              <div style={{fontSize:11,fontWeight:700,color:'var(--text3)',letterSpacing:'0.07em',
                textTransform:'uppercase' as const,marginBottom:8}}>Suggested Questions</div>
              <div style={{display:'flex',flexWrap:'wrap',gap:6}}>
                {SUGGESTED.map((s,i)=>(
                  <button key={i} onClick={()=>handleChat(s)}
                    style={{padding:'7px 12px',borderRadius:7,border:'1px solid var(--border)',
                      background:'var(--surface2)',fontSize:11.5,color:'var(--text2)',fontFamily:'inherit',
          ...
          <div style={{flex:1,overflowY:'auto',marginBottom:16,minHeight:300,display:'flex',flexDirection:'column',gap:12}}>
            {messages.map((m,i)=>(
